...
// We only ever talk to it over DeviceIoControl, never through PawnIOLib,
// which is 64 bit only and would not link into this 32 bit build anyway.
//
// The module is not shipped with the program. PawnIO is installed by the
// user, same as the port driver is today, and the module blob is looked
// for next to the exe. Without one we simply report failure and the
// caller stays on the driver it used before.
...
// ours covers every port the program uses, the stock one only the legacy window
static const char* const MODULE_OURS = "LpcThinkpadEC.bin";
...
bool PawnIo_Open() {
	...
	if (LoadModule(MODULE_OURS))
		g_module = MODULE_OURS;
	else if (LoadModule(MODULE_STOCK))
...
bool PawnIo_PortSupported(USHORT port) {
	...
	if (port == 0x1600 || port == 0x1604 || (port >= 0x1610 && port <= 0x161f))
		return g_module == MODULE_OURS;
